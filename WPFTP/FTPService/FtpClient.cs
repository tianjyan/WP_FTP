...
namespace WPFTPService.FtpService
{
    public class FtpClient
    {
        ...
        public string FtpServerAddress = String.Empty;
        ...
        public FtpClient(String FtpServerIpAddress, String PortNumber, String UserName, String Password,Dispatcher UIDispatcher)
        {
            this.UIDispatcher = UIDispatcher;
            this.FtpServerAddress = FtpServerIpAddress;
            this.FtpServerPort = PortNumber;
            ...
            switch (ftpPassiveOperation)
            {
                ...
                case FtpPassiveOperation.ListDirectory:
                    ...
                    if (Response.StartsWith("150") || Response.StartsWith("125"))
                    {
                        ...
                        String listingData = encoding.GetString(fileListingData.ToArray(), 0, fileListingData.ToArray().Length);
        ...
        {
            ChannelInfo = ChannelInfo.Remove(0, "227 Entering Passive Mode".Length);		//可能不同的FTP返回的数据不同,可能出现问题,待测
            //获取端口号
            String[] Splits = ChannelInfo.Substring(ChannelInfo.IndexOf("(") + 1, ChannelInfo.Length - ChannelInfo.IndexOf("(") - 3).Split(new char[] { ',', ' ', }, StringSplitOptions.RemoveEmptyEntries);
            ...
            await FtpDataChannel.ConnectAsync(new Windows.Networking.HostName(this.FtpServerAddress), port.ToString());
