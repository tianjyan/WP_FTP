...
namespace WPFTP
{
    public partial class App : Application
    {
        ...
        public async Task ConnectToFtpServer(String FtpServerIpAddress, String PortNumber, String UserName, String Password)
        {
            if (Client == null)
            {
                Client = new FtpClient(FtpServerIpAddress, PortNumber,UserName,Password, RootFrame.Dispatcher);
                Client.FtpAuthenticationFailed += ftpClient_FtpAuthenticationFailed;
                Client.FtpAuthenticationSucceeded += ftpClient_FtpAuthenticationSucceeded;
                Client.FtpConnected += ftpClient_FtpConnected;
                Client.FtpDirectoryChangedFailed += ftpClient_FtpDirectoryChangedFailed;
                Client.FtpDirectoryChangedSucceded += ftpClient_FtpDirectoryChangedSucceded;
                //Client.FtpDirectoryListed += ftpClient_FtpDirectoryListed;
                Client.FtpDisconnected += ftpClient_FtpDisconnected;
                Client.FtpFileDownloadFailed += ftpClient_FtpFileDownloadFailed;
                Client.FtpFileDownloadSucceeded += ftpClient_FtpFileDownloadSucceeded;
                Client.FtpFileTransferProgressed += ftpClient_FtpFileTransferProgressed;
                Client.FtpFileUploadFailed += ftpClient_FtpFileUploadFailed;
                Client.FtpFileUploadSucceeded += ftpClient_FtpFileUploadSucceeded;
                Client.FtpPresentWorkingDirectoryReceived += ftpClient_FtpPresentWorkingDirectoryReceived;
        ...
        void ftpClient_FtpPresentWorkingDirectoryReceived(object sender, FtpPresentWorkingDirectoryEventArgs e)
        {
            ...
            {
                ServerCurrentWorkingDirectory = e.PresentWorkingDirectory;
            });
        }
        ...
        void ftpClient_FtpDisconnected(object sender, FtpDisconnectedEventArgs e)
        {
            RootFrame.Dispatcher.BeginInvoke(() =>
            {
                MessageBox.Show(String.Format("You have been disconnected. Reason: {0}", e.DisconnectReason), "WPFTP Demo App", MessageBoxButton.OK);
            });
        }
        ...
        async void ftpClient_FtpDirectoryChangedSucceded(object sender, FtpDirectoryChangedEventArgs e)
        {
            await Client.GetPresentWorkingDirectoryAsync();
        }
        ...
        async void ftpClient_FtpAuthenticationSucceeded(object sender, EventArgs e)
        {
            await Client.GetPresentWorkingDirectoryAsync();
            RootFrame.Dispatcher.BeginInvoke(() =>
            {
                RootFrame.Navigate(new Uri("/FtpDirectoryBrowser.xaml", UriKind.Relative));
            });
        }
        ...
        void ftpClient_FtpAuthenticationFailed(object sender, EventArgs e)
        {
            RootFrame.Dispatcher.BeginInvoke(() =>
            {
                MessageBox.Show("Login Unsuccessful", "WPFTP Demo App", MessageBoxButton.OK);
            });
        }
